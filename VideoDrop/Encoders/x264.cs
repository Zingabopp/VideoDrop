...
{
    public class x264 : VideoDropEncoder
    {
        private x264_Options _currentSettings;
        ...
        public static List<string> ValidTypes
        {
            get
            {
                return new List<string>() { ".h264", ".mkv", ".flv", ".mp4" };
            }
        }
        ...
        public override string OutputType
        {
            ...
            set
            {
                // Check if value is a valid output type, if it's not, use .h264 as default
                _currentSettings.OutputType = OutputTypes.Contains(value) ? value : ".h264";
            }
        }
        ...
        public override string Arguments
        {
            ...
            {
                _arguments = value;
                WriteInfo(this.GetType().Name + "." + GetMethodName(), "Parameters for x264: " + value, DEBUGLEVEL.INFO);
            }
        }
        public x264(Dictionary<string, string> encoderSettings) : base(encoderSettings)
        {
            _currentSettings.Settings = encoderSettings;
        }

        public x264(string inputFile, string outputFile, string workingFolder, Dictionary<string, string> encoderSettings) : base(inputFile, outputFile, workingFolder, encoderSettings)
        {
            _currentSettings = new x264_Options();
            _currentSettings.Settings = encoderSettings;
        ...
        // _validExePath = ExecutablePath.EndsWith("x264.exe")
        // Arguments = args
        // End Sub
        ...
        public override string BuildEncoderArguments()
        {
            ...
            args = args + " -o " + OutputFile + " " + InputFile;
    ...
    public class x264_Options
    {
        ...
        public string EncoderPath
        {
            get
            {
                ...
                if (_settings.ContainsKey("EncoderPath"))
                {
                    ...
                    {
                        if (_settings["EncoderPath"].EndsWith("x264.exe"))
            ...
            set
            {
                ...
                {
                    if (value.ToLower().EndsWith("x264.exe"))
        ...
        public string OutputType
        {
            ...
            set
            {
                if (!value.StartsWith("."))
                    value = "." + value;
                if (x264.ValidTypes.Contains(value))
                    ModifyDictionary("outputType", value, ref _settings);
                else
                    WriteInfo(this.GetType().Name + "." + GetMethodName(), "Invalid output type for x264: " + value, DEBUGLEVEL.ERROR);
            }
        }
        ...
        public string cpuPreset
        {
            get
            {
                ...
                {
                    WriteInfo(this.GetType().Name + "." + GetMethodName(), "No preset found for x264, using medium", DEBUGLEVEL.ERROR);
                    return "medium";
                }
            }
            set
            {
                if (x264.ValidTypes.Contains(value))
                    ...
                    WriteInfo(this.GetType().Name + "." + GetMethodName(), "Invalid preset for x264: " + value, DEBUGLEVEL.ERROR);
            }
        }
        public int crf
        {
        ...
        public override string ToString()
        {
            ...
            WriteInfo(this.GetType().Name + "." + GetMethodName(), "x264_Options ToString(): " + retStr.Trim(), DEBUGLEVEL.INFO);
            return retStr.Trim();
        }
    }
